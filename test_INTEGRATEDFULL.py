...
##    1. LINEAR    (default / legacy)  -> a single straight line over the whole working range
##    2. PIECEWISE (optional)          -> several straight lines, one per concentration band
##
##  Each segment is stored in CONCENTRATION space (that is how you fit it in the lab:
##  you prepare standards of known concentration and regress i = m*C + b for that band).
##  At import time these are converted into a set of non-overlapping CURRENT intervals,
##  because at run time the only thing the instrument gives us is a current.
##
##  Add as many segments as you like - just keep them in ascending concentration order.
############################################################################################

CALIBRATION_SEGMENTS = {
    'Catechol': [
        {
            'conc_range': (0.0, 100.0),      # uM  (low  band - highest sensitivity)
            'slope':      0.1499,            # uA / uM
            'intercept': -0.4881             # uA
        },
        {
            'conc_range': (100.0, 500.0),    # uM  (mid  band)
            'slope':      0.1245,            # uA / uM
            'intercept':  2.1543             # uA
        },
        {
            'conc_range': (500.0, 2000.0),   # uM  (high band - sensitivity rolls off)
            'slope':      0.0987,            # uA / uM
            'intercept':  8.7654             # uA
        }
    ]
...
# Single-line ("classic") calibration, used when use_piecewise=False.
LINEAR_COEFFS = {
    'Catechol': {
        'slope':      0.1499,                # uA / uM
        'intercept': -0.4881,                # uA
        'conc_range': (0.0, 500.0)           # uM - validated working range of the single line
    }
}

OUT_OF_RANGE_MESSAGE = (
    "OUT OF RANGE, PERFORM SUCCESSIVE DILUTIONS UNTIL YOU ACHIEVE A CURRENT READING "
    "THAT IS IN RANGE. MULTIPLY THE CORRESPONDING CONCENTRATION BY THE DILUTION "
    "FACTOR TO OBTAIN THE TRUE CONCENTRATION."
)


def build_current_breakpoints(segments):
    """
    Convert concentration-domain segments into non-overlapping current-domain intervals.

    Why: the segments are fitted in concentration space, but at run time we only know the
    current. Selecting the segment by "compute C with every line and see which one lands in
    its own band" is ambiguous - independently fitted lines are not continuous, so near a
    join two segments can both claim the point (overlap) or neither can (gap).

    Fix: for every segment compute the current at its two concentration endpoints
    (i = m*C + b). Sort segments by current. Wherever two neighbours meet, put the
    breakpoint at the MIDPOINT of the two boundary currents. That closes gaps and removes
    overlaps, and every current maps to exactly one segment.

    Returns a list of dicts sorted by current, each with:
        index, slope, intercept, conc_range, i_lo, i_hi   (i_lo/i_hi = effective bounds, uA)
    """
    prepared = []
    for idx, seg in enumerate(segments):
        c_lo, c_hi = seg['conc_range']
        m = float(seg['slope'])
        b = float(seg['intercept'])
        if m == 0:
            raise ValueError(f"Segment {idx} has zero slope - not invertible.")
        i_a = m * c_lo + b
        i_b = m * c_hi + b
        prepared.append({
            'index':      idx,
            'slope':      m,
            'intercept':  b,
            'conc_range': (float(c_lo), float(c_hi)),
            'i_raw':      (min(i_a, i_b), max(i_a, i_b))
        })

    prepared.sort(key=lambda s: s['i_raw'][0])

    for j in range(len(prepared) - 1):
        upper = prepared[j]['i_raw'][1]        # top current of this segment
        lower = prepared[j + 1]['i_raw'][0]    # bottom current of the next segment
        breakpoint_i = 0.5 * (upper + lower)   # midpoint => no gap, no overlap
        prepared[j]['i_hi'] = breakpoint_i
        prepared[j + 1]['i_lo'] = breakpoint_i

    prepared[0]['i_lo'] = prepared[0]['i_raw'][0]
    prepared[-1]['i_hi'] = prepared[-1]['i_raw'][1]
    return prepared


# Pre-computed once at import; rebuild if you edit CALIBRATION_SEGMENTS at run time.
CURRENT_BREAKPOINTS = {
    name: build_current_breakpoints(segs) for name, segs in CALIBRATION_SEGMENTS.items()
}


def calibration_current_window(substance='Catechol', use_piecewise=True):
    """Return (i_min, i_max) in uA: the currents this calibration is allowed to interpret."""
    if use_piecewise and substance in CURRENT_BREAKPOINTS:
        bps = CURRENT_BREAKPOINTS[substance]
        return bps[0]['i_lo'], bps[-1]['i_hi']
    coeffs = LINEAR_COEFFS[substance]
    c_lo, c_hi = coeffs['conc_range']
    i_a = coeffs['slope'] * c_lo + coeffs['intercept']
    i_b = coeffs['slope'] * c_hi + coeffs['intercept']
    return min(i_a, i_b), max(i_a, i_b)
...
def get_concentration_from_current(current,
                                   substance='Catechol',
                                   use_piecewise=True,
                                   dilution_factor=1.0,
                                   segments=None):
    """
    Convert a measured current into a concentration.

    Parameters
    ----------
    current : float
        Measured peak / steady-state current in uA.
    substance : str
        Key into CALIBRATION_SEGMENTS / LINEAR_COEFFS. Default 'Catechol'.
    use_piecewise : bool
        True  -> piecewise linear calibration (segment chosen by measured-current breakpoints)
        False -> single linear calibration from LINEAR_COEFFS
    dilution_factor : float
        Factor the sample was diluted by before measurement (1.0 = neat sample).
        The reported concentration is  raw_concentration * dilution_factor.
    segments : list or None
        Optional override of CALIBRATION_SEGMENTS[substance] (breakpoints are rebuilt).

    Returns
    -------
    dict
        concentration     : float or None  - TRUE concentration in uM (raw * dilution_factor),
                                             None when the reading is out of range
        raw_concentration : float or None  - concentration of the measured (diluted) solution
        dilution_factor   : float
        segment_index     : int   - index of the segment used, -1 if none / linear mode
        in_range          : bool  - False when the current falls outside the calibrated window
        method            : 'piecewise' | 'linear'
        unit              : 'uM'
        message           : str   - human readable status (dilution instructions if out of range)
    """
    current = float(current)
    dilution_factor = float(dilution_factor) if dilution_factor else 1.0
    ...
    result = {
        ...
        'segment_index':     -1,
        'in_range':          False,
        'method':            'piecewise' if use_piecewise else 'linear',
        'unit':              'uM',
        'message':           ''
    }

    if substance not in CALIBRATION_SEGMENTS and substance not in LINEAR_COEFFS:
        result['message'] = f"No calibration data available for '{substance}'."
    ...
    if use_piecewise:
        if segments is not None:
            breakpoints = build_current_breakpoints(segments)
        else:
            breakpoints = CURRENT_BREAKPOINTS[substance]

        for bp in breakpoints:
            if bp['i_lo'] <= current <= bp['i_hi']:
                raw = (current - bp['intercept']) / bp['slope']
                result['raw_concentration'] = raw
                result['concentration'] = raw * dilution_factor
                result['segment_index'] = bp['index']
                result['in_range'] = True
                result['message'] = (
                    f"Segment {bp['index'] + 1} "
                    f"({bp['conc_range'][0]:.0f}-{bp['conc_range'][1]:.0f} uM, "
                    f"m={bp['slope']}, b={bp['intercept']})"
                )
                print(f"[CAL] Piecewise segment {bp['index'] + 1} | "
                      f"I = {current:.4f} uA -> C_raw = {raw:.2f} uM | "
                      f"DF = {dilution_factor:g} -> C_true = {result['concentration']:.2f} uM")
                return result

        i_min, i_max = breakpoints[0]['i_lo'], breakpoints[-1]['i_hi']
        result['message'] = OUT_OF_RANGE_MESSAGE
        print(f"[CAL] {current:.4f} uA is outside the calibrated window "
              f"[{i_min:.4f}, {i_max:.4f}] uA")
        print(f"[CAL] {OUT_OF_RANGE_MESSAGE}")
        return result
    ...
    if c_lo <= raw <= c_hi:
        ...
        result['concentration'] = raw * dilution_factor
        result['in_range'] = True
        result['message'] = f"Linear calibration (m={m}, b={b}) over {c_lo:.0f}-{c_hi:.0f} uM"
        print(f"[CAL] Linear | C_raw = ({current:.4f} - {b}) / {m} = {raw:.2f} uM | "
              f"DF = {dilution_factor:g} -> C_true = {result['concentration']:.2f} uM")
        return result

    result['message'] = OUT_OF_RANGE_MESSAGE
    print(f"[CAL] Linear estimate {raw:.2f} uM lies outside the validated range "
          f"[{c_lo:.0f}, {c_hi:.0f}] uM")
    print(f"[CAL] {OUT_OF_RANGE_MESSAGE}")
    return result
...
def classify_safety(concentration):
    ...
    Note: `concentration` may be None when the calibration reported OUT OF RANGE.
    In that case no safety verdict can be given until the sample is diluted.
    """
    ...
    if concentration is None:
        return {
            'status': 'OUT OF RANGE',
            'color': '#455a64',  # Blue-grey - deliberately not a safety colour
            'description': OUT_OF_RANGE_MESSAGE
        }
...
def startCV():
    ...
    if (variable_SUBSTANCE.get()=='Catechol'):
        ########## ===== INTEGRATED PIECEWISE / LINEAR CALIBRATION ===== ############
        W = 110.1   # Molecular weight (g/mol)

        # ===== Which calibration method? =====
        # Driven by the "Piecewise calibration" checkbox in the interface.
        # Checked   -> piecewise linear (segment picked from measured-current breakpoints)
        # Unchecked -> classic single linear calibration
        use_piecewise = bool(variable_PIECEWISE.get())

        # ===== Dilution factor entered by the operator (1 = neat sample) =====
        dilution_factor = get_dilution_factor()

        # The current we hand to the calibration depends on the measurement method
        if ("{}".format(variable_METHOD.get()) == "Fixed Voltage"):
            measured_current = max_current_val
            method_label = "FIXED VOLTAGE (CA)"
        elif ("{}".format(variable_METHOD.get()) == "Cyclic Voltammetry"):
            # Catechol oxidation peak: use the larger of the two peaks in magnitude
            measured_current = max(peak_Anodic_Current, peak_Cathodic_Current, key=abs)
            method_label = "CYCLIC VOLTAMMETRY (CV)"
        else:
            measured_current = None
            method_label = "UNKNOWN METHOD"
        ...
        if measured_current is not None:
            ...
            Concentration = cal_result['concentration']          # true conc (raw x DF), or None
            raw_conc      = cal_result['raw_concentration']      # conc of the measured solution
            method_used   = cal_result['method']
            in_range      = cal_result['in_range']

            print(f"\n{'='*70}")
            print(f"CONCENTRATION CALCULATION - {method_label}")
            print(f"{'='*70}")
            print(f"Measured Current    : {measured_current:.6f} uA")
            ...
            if in_range:
                print(f"Measured (diluted)  : {raw_conc:.2f} uM")
                print(f"TRUE Concentration  : {Concentration:.2f} uM")
                print(f"{'='*70}\n")

                results.insert('1.0', f">> {method_used.upper()} calibration | {cal_result['message']}\n")
                results.insert('1.0', f"Measured (diluted) = {raw_conc:.2f} uM | DF = {dilution_factor:g}\n")
                results.insert('1.0', f"Catechol Concentration(uM) = {Concentration:.2f}\n")

                safety = classify_safety(Concentration)
                results.insert('1.0', f"STATUS: {safety['status']}\n\n")
                w.insert('1.0', f">> {safety['description']}\n\n")
            else:
                print(f"RESULT              : OUT OF RANGE")
                print(f"{'='*70}\n")
                results.insert('1.0', f">> {OUT_OF_RANGE_MESSAGE}\n")
                results.insert('1.0', f"Catechol Concentration(uM) = OUT OF RANGE\n\n")
                w.insert('1.0', f">> {OUT_OF_RANGE_MESSAGE}\n\n")

            # ===== Hand the measurement to the touchscreen app =====
            # current.txt is polled by "modern app interface.py". It is now written for BOTH
            # measurement methods (previously only Fixed Voltage wrote it, so a CV run left the
            # touchscreen app reading a stale value).
            try:
                with open("/home/plaksha/touchscreen_app/current.txt", "w") as f_current:
                    f_current.write(f"{measured_current:.8f}\n")
                print("Measured current saved to current.txt")
            except Exception as e:
                w.insert('1.0', f"Error saving current to current.txt: {e}\n\n")
                print(f"Error saving current to current.txt: {e}")
...
# ===== NEW: calibration mode + dilution factor =====
variable_PIECEWISE = IntVar(root)
variable_PIECEWISE.set(1)          # 1 = piecewise linear (default), 0 = classic single linear

PIECEWISE_check = Checkbutton(root,
                              text='Piecewise calibration  (uncheck for single linear)',
                              variable=variable_PIECEWISE)

DILUTION_label = Label(root, text='Dilution factor')
DILUTION_entry = Entry(root, width='10')
DILUTION_entry.insert(0, '1')


def get_dilution_factor():
    """
    Read the dilution factor from the interface.

    The operator dilutes the sample when a reading comes back OUT OF RANGE. The reported
    concentration is then  (concentration of the diluted solution) x (dilution factor).
    A blank / invalid / non-positive entry is treated as 1 (neat sample).
    """
    try:
        df = float(DILUTION_entry.get())
        if df <= 0:
            raise ValueError
        return df
    except (ValueError, TclError):
        w.insert('1.0', ">> Invalid dilution factor - assuming 1 (neat sample)\n\n")
        return 1.0
...
def compute_stock_addition_schedule(target_concentrations,
                                    initial_volume,
                                    stock_concentration,
    ...
    exact stock aliquot to pipette at each step.

    Recursive formula (concentrate-only workflow):

        V_stock,t+1 = V_0 * (C_(t+1) - C_t) * (C_stock - C_0)
                      -----------------------------------------
                      (C_stock - C_(t+1)) * (C_stock - C_t)

        V_(t+1)     = V_t + V_stock,t+1

    (Equivalent step form: V_stock,t+1 = V_t * (C_(t+1) - C_t) / (C_stock - C_(t+1)),
    since V_t = V_0 * (C_stock - C_0) / (C_stock - C_t) by induction.)

    The concentration actually reached at each step is re-derived from the pipetted
    volumes (mole balance: n_i = n_(i-1) + V_add,i * C_stock ; C_achieved = n_i / V_i)
    and returned alongside the target, so any arithmetic or pipetting inconsistency is
    visible at a glance.

    Physical constraint surfaced to the user: Stock concentration > every target
    concentration. A target cannot be reached by spiking in a stock that is not more
    concentrated than that target; the schedule is refused with an explanatory message
    rather than returning a negative or infinite volume.

    Parameters
    ----------
    target_concentrations : sequence of float
        Ascending list of desired concentrations (e.g. uM). Each must exceed the
        previous one and the initial (background) concentration.
    initial_volume : float
        Starting volume in the cell (e.g. mL). Must be > 0.
    stock_concentration : float
        Concentration of the spiking stock (same unit as targets). Must be strictly
        greater than every target.
    initial_concentration : float, optional
        Background concentration already in the cell (0 for clean buffer).

    Returns
    -------
    dict with keys:
        'ok'             : bool  - True if a valid schedule was produced
        'message'        : str   - human-readable status / refusal reason
        'steps'          : list of dicts, one per step:
                           {'step', 'target_conc', 'add_volume', 'total_volume',
                            'achieved_conc'}
        'total_stock'    : float - total stock volume consumed
        'final_volume'   : float - running total volume after the last step
    """
    result = {'ok': False, 'message': '', 'steps': [],
              'total_stock': 0.0, 'final_volume': None}

    # ---- Input validation -------------------------------------------------------
    try:
        targets = [float(c) for c in target_concentrations]
        V0 = float(initial_volume)
        Cs = float(stock_concentration)
        C0 = float(initial_concentration)
    except (TypeError, ValueError):
        result['message'] = 'Invalid numeric input.'
        return result

    if len(targets) == 0:
        result['message'] = 'No target concentrations were provided.'
        return result
    if V0 <= 0:
        result['message'] = 'Initial volume must be positive.'
        return result
    if C0 < 0:
        result['message'] = 'Background concentration cannot be negative.'
        return result

    previous = C0
    for c in targets:
        if c <= previous:
            result['message'] = ('Target concentrations must be strictly ascending '
                                 'and above the background concentration '
                                 '(offending target: %g).' % c)
            return result
        previous = c

    # Physical constraint: stock must be MORE concentrated than every target.
    if Cs <= max(targets):
        result['message'] = ('SCHEDULE REFUSED: Stock concentration (%g) must be '
                             'strictly greater than every target concentration '
                             '(highest target: %g). A target cannot be reached by '
                             'spiking in a stock that is not more concentrated than '
                             'that target. This planner implements the concentrate-only '
                             'workflow appropriate to catechol standard addition.'
                             % (Cs, max(targets)))
        return result
    ...
    V_prev = V0
    C_prev = C0
    n_moles = C0 * V0          # analyte amount (conc x volume, consistent units)
    total_stock = 0.0
    ...
    for idx, C_i in enumerate(targets, start=1):
        ...
        V_i = V_prev + V_add
        n_moles = n_moles + V_add * Cs                   # mole balance
        C_achieved = n_moles / V_i                       # re-derived concentration
        total_stock += V_add

        result['steps'].append({'step': idx,
                                'target_conc': C_i,
                                'add_volume': V_add,
                                'total_volume': V_i,
                                'achieved_conc': C_achieved})
        V_prev = V_i
        C_prev = C_i

    result['ok'] = True
    result['total_stock'] = total_stock
    result['final_volume'] = V_prev
    result['message'] = ('Schedule computed: %d step(s), %.4f total stock consumed, '
                         'final volume %.4f.' % (len(targets), total_stock, V_prev))
    return result


def compute_stabilization_time(current,
                               time_axis=None,
                               tolerance=0.02,
                               sustained_samples=5,
                               guard_fraction=0.05,
    ...
    second derivative of current with respect to time - tends to zero:

        stable when   d2i/dt2 -> 0   AND   di/dt -> 0

    A naive 'second derivative equals zero' test is insufficient on its own, because
    an inflection point on the decaying transient also has zero curvature while the
    slope is still steep. This function therefore requires BOTH the normalised
    curvature and the normalised slope to fall below a small fraction of their own
    peak magnitudes (default tolerance 2%) and to remain there for a sustained window
    (default 5 consecutive samples). Both derivatives are obtained from a
    Savitzky-Golay filter (smooths and differentiates in a single pass), and an
    initial guard region (default: first 5% of the record) is skipped so the large
    opening transient is never mistaken for a plateau.

    Parameters
    ----------
    current : sequence of float
        Current-versus-time record (e.g. the 400-sample Fixed Voltage transient).
    time_axis : sequence of float, optional
        Timestamps of each sample. If omitted, a uniform 1 s spacing is assumed
        (matching the development interface's Fixed Voltage sampling of sweep()).
    tolerance : float, optional
        Fraction of each derivative's own peak magnitude below which it is
        considered "zero" (default 0.02, i.e. 2%).
    sustained_samples : int, optional
        Number of consecutive samples the joint criterion must hold (default 5).
    guard_fraction : float, optional
        Fraction of the record skipped at the start (default 0.05, i.e. 5%).
    savgol_window : int, optional
        Savitzky-Golay window length in samples (odd; clipped to the record length).
    ...
    -------
    dict with keys:
        'settled'            : bool  - True if a sustained plateau was found
        'stabilization_time' : float or None - time of the FIRST sample satisfying
                               the joint criterion (measure here)
        'index'              : int or None   - sample index of that time
        'message'            : str  - status, or the recommendation to use a longer
                               measurement / looser threshold when not settled
    """
    out = {'settled': False, 'stabilization_time': None, 'index': None, 'message': ''}

    i_arr = np.asarray(current, dtype=float)
    n = i_arr.size
    if n < max(savgol_polyorder + 2, 7):
        out['message'] = 'Record too short for stabilisation analysis.'
        return out

    if time_axis is None:
        t_arr = np.arange(n, dtype=float)          # 1 s per sample default
    else:
        t_arr = np.asarray(time_axis, dtype=float)
        if t_arr.size != n:
            out['message'] = 'time_axis length does not match current record.'
            return out
    dt = float(np.median(np.diff(t_arr)))
    if not np.isfinite(dt) or dt <= 0:
        out['message'] = 'Invalid time axis (non-increasing or non-finite).'
        return out

    # Savitzky-Golay: smooth + differentiate in a single pass.
    ...
    if win <= savgol_polyorder:
        out['message'] = 'Record too short for the requested Savitzky-Golay window.'
        return out

    d1 = savgol_filter(i_arr, window_length=win, polyorder=savgol_polyorder,
                       deriv=1, delta=dt)
    d2 = savgol_filter(i_arr, window_length=win, polyorder=savgol_polyorder,
                       deriv=2, delta=dt)

    # Normalise each derivative by its own peak magnitude.
    peak_d1 = float(np.max(np.abs(d1)))
    peak_d2 = float(np.max(np.abs(d2)))
    if peak_d1 == 0.0 and peak_d2 == 0.0:
        # Perfectly flat record: already at steady state after the guard region.
        guard = int(np.ceil(guard_fraction * n))
        out.update(settled=True, index=guard,
                   stabilization_time=float(t_arr[guard]),
                   message='Record is flat; steady state from t = %.2f.'
                           % float(t_arr[guard]))
        return out

    ok_d1 = (np.abs(d1) <= tolerance * peak_d1) if peak_d1 > 0 else np.ones(n, bool)
    ok_d2 = (np.abs(d2) <= tolerance * peak_d2) if peak_d2 > 0 else np.ones(n, bool)
    joint = ok_d1 & ok_d2

    # Guard region: never mistake the opening transient for a plateau.
    guard = int(np.ceil(guard_fraction * n))
    joint[:guard] = False

    # First sample where the joint criterion holds for `sustained_samples` in a row.
    run = 0
    for idx in range(n):
        run = run + 1 if joint[idx] else 0
        if run >= int(sustained_samples):
            first = idx - int(sustained_samples) + 1
            out.update(settled=True, index=first,
                       stabilization_time=float(t_arr[first]),
                       message='Plateau reached: stabilisation time = %.2f '
                               '(joint |di/dt| and |d2i/dt2| below %.0f%% of their '
                               'peaks for %d consecutive samples).'
                               % (float(t_arr[first]), 100.0 * tolerance,
                                  int(sustained_samples)))
            return out

    out['message'] = ('Transient has NOT settled within the record. Recommend a '
                      'longer measurement or a looser threshold rather than taking '
                      'a premature value.')
    return out


def compute_time_between_measurements(t_stabilization,
                                      t_mix=30.0,
                                      safety_margin=1.2):
    """
    ...
    homogenise throughout the stirred cell (mixing time, t_mix - operator-entered;
    30 s is a typical bench default for a small, well-stirred cell), and (2) the
    electrode must re-equilibrate to the new concentration, which is precisely the
    stabilisation time of the resulting transient (from compute_stabilization_time).
    A conservative safety margin (default 1.2) is applied:

        Time between measurements = safety_margin * (t_stabilization + t_mix)

    This prevents premature spikes (that would corrupt the next reading) and avoids
    ...
    dict with keys:
        'ok'      : bool
        'wait_s'  : float or None - recommended wait before the next spike
        'message' : str
    """
    out = {'ok': False, 'wait_s': None, 'message': ''}
    try:
        t_stab = float(t_stabilization)
        t_m = float(t_mix)
        sm = float(safety_margin)
    except (TypeError, ValueError):
        out['message'] = 'Invalid numeric input.'
        return out
    if t_stab < 0 or t_m < 0:
        out['message'] = 'Times cannot be negative.'
        return out
    if sm < 1.0:
        out['message'] = ('Safety margin must be >= 1.0 (a margin below 1 would '
                          'schedule the next spike before mixing and '
                          're-equilibration are guaranteed complete).')
        return out
    wait = sm * (t_stab + t_m)
    out.update(ok=True, wait_s=wait,
               message='Wait %.1f s = %.2f x (t_stabilisation %.1f s + t_mix %.1f s) '
                       'before adding the next stock aliquot.'
                       % (wait, sm, t_stab, t_m))
    return out


############################################################################################
## ===== NEW: CHRONOAMPEROMETRY PLANNER UI (dedicated input panel + menu action) =====
...
def openChronoPlanner():
    ...
    background concentration (0 for clean buffer). The volume schedule (6.2.1) is
    available immediately from the typed inputs; the stabilisation time (6.2.2) and
    the time between measurements (6.2.3) appear once a Fixed Voltage
    (chronoamperometry) run has produced a transient in DATA_fv.
    """
    planner = Toplevel(root)
    planner.title('Chronoamperometry Planner')
    planner.geometry('720x560')

    # ---- Input panel -----------------------------------------------------------
    Label(planner, text='Target concentrations (ascending, comma-separated, uM)'
          ).grid(column=0, row=0, sticky='w', padx=8, pady=(10, 2))
    ca_targets_entry = Entry(planner, width=40)
    ca_targets_entry.insert(0, '5, 10, 20, 30')
    ca_targets_entry.grid(column=1, row=0, padx=8, pady=(10, 2), sticky='w')

    Label(planner, text='Initial volume (mL)').grid(column=0, row=1, sticky='w', padx=8, pady=2)
    ca_volume_entry = Entry(planner, width=12)
    ca_volume_entry.insert(0, '10')
    ca_volume_entry.grid(column=1, row=1, padx=8, pady=2, sticky='w')

    Label(planner, text='Stock concentration (uM)').grid(column=0, row=2, sticky='w', padx=8, pady=2)
    ca_stock_entry = Entry(planner, width=12)
    ca_stock_entry.insert(0, '1000')
    ca_stock_entry.grid(column=1, row=2, padx=8, pady=2, sticky='w')

    Label(planner, text='Background concentration (uM, 0 = clean buffer)'
          ).grid(column=0, row=3, sticky='w', padx=8, pady=2)
    ca_background_entry = Entry(planner, width=12)
    ca_background_entry.insert(0, '0')
    ca_background_entry.grid(column=1, row=3, padx=8, pady=2, sticky='w')

    Label(planner, text='Mixing time t_mix (s)').grid(column=0, row=4, sticky='w', padx=8, pady=2)
    ca_tmix_entry = Entry(planner, width=12)
    ca_tmix_entry.insert(0, '30')
    ca_tmix_entry.grid(column=1, row=4, padx=8, pady=2, sticky='w')

    Label(planner, text='Safety margin').grid(column=0, row=5, sticky='w', padx=8, pady=2)
    ca_margin_entry = Entry(planner, width=12)
    ca_margin_entry.insert(0, '1.2')
    ca_margin_entry.grid(column=1, row=5, padx=8, pady=2, sticky='w')

    ca_output = Text(planner, width=86, height=20, bg='white', relief='groove')
    ca_output.grid(column=0, row=7, columnspan=2, padx=8, pady=8)

    # ---- Handlers (new, nested; touch nothing outside this panel) ---------------
    def _ca_show(text):
        ca_output.insert('1.0', text + '\n')

    def _ca_compute_schedule():
        raw = ca_targets_entry.get()
        try:
            targets = [float(tok) for tok in raw.replace(';', ',').split(',')
                       if tok.strip() != '']
        except ValueError:
            _ca_show('>> Could not parse target concentrations: "%s"' % raw)
            return
        sched = compute_stock_addition_schedule(
            targets,
            initial_volume=ca_volume_entry.get(),
            stock_concentration=ca_stock_entry.get(),
            initial_concentration=ca_background_entry.get())
        if not sched['ok']:
        ...
        for s in sched['steps']:
            lines.append('%-6d %-14.4f %-14.4f %-12g %-14.4f'
                         % (s['step'], s['add_volume'], s['total_volume'],
                            s['target_conc'], s['achieved_conc']))
        lines.append('Total stock consumed: %.4f mL | Final volume: %.4f mL'
                     % (sched['total_stock'], sched['final_volume']))
        _ca_show('\n'.join(lines) + '\n')

    def _ca_analyze_transient():
        # Uses the most recent Fixed Voltage (chronoamperometry) transient.
        try:
            transient = np.asarray(DATA_fv, dtype=float)
            times = np.asarray(t_fv, dtype=float) if 't_fv' in globals() else None
        except (NameError, TypeError):
            _ca_show('>> No chronoamperometry transient available yet. Run a '
                     'Fixed Voltage measurement first (Method = Fixed Voltage, '
                     'then Start).')
            return
        if transient.size == 0 or not np.any(transient != 0):
    ...
    Button(planner, text='Compute Volume Schedule',
           command=_ca_compute_schedule).grid(column=0, row=6, padx=8, pady=6, sticky='w')
    Button(planner, text='Analyze Last CA Transient (stabilisation + spike timing)',
    ...
    ca_output.insert('1.0',
                     ...
                     'stabilisation time and the wait before the next spike.\n\n')


# ===== NEW: surface the planner as a menu action (additive; existing menu untouched) =====
menubar.add_command(label='Chronoamperometry Planner', command=openChronoPlanner)
...
PIECEWISE_check.grid(column='1',row='6',columnspan='2',rowspan='1')
DILUTION_label.grid(column='3',row='6',columnspan='1',rowspan='1')
DILUTION_entry.grid(column='3',row='7',columnspan='1',rowspan='1')
